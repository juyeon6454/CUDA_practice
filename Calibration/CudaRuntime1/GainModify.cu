...
// gain, dark 합산 및 평균 내는 거 따로 함수 만들기
...
__global__ void cudaPixelAvg(float* averageimg, ushort* outimg, int width, int height)
{
    ...
    if (x < width && y < height) {
        *(outimg + offset) = *(averageimg + offset) / 30;
    }
}
    ...
    int width, int height, ushort* outimg)
{
    ...
    if (x < width && y < height) {
        *(outimg + offset) =
        ...
        //(float)((*(GainMapImg + offset)) - (*(darkMapImg + offset))) * subGainAvg;
    }
}
...
void dirGainfile()
{
    ...
    string file = "C:/calibration_mentoring/pano/Gain_No_Filter/60kV_10mA";
    //char path[] = file.c_str();
